import asyncio, discord, json, psutil, os, tictactoe, ultimateTTT, connectfour, rockpaperscissors, chess
...
@client.command(
    name="tictactoe",
    help="Select a space to mark with your character and attempt to get 3 in a row before your opponent does",
    usage="@user",
    aliases=["ttt"],
    description="A simple game of Tic Tac Toe",
)
async def tictactoeCommand(ctx, user: discord.User):
    if not user == None:
...
@client.command(
    ...
    help="https://en.wikipedia.org/wiki/Ultimate_tic-tac-toe",
    usage="@user",
    aliases=["ultimatettt", "utictactoe", "uttt"],
    description="A complex game of Tic Tac Toe, i highly recommend reading the wikipedia page before playing",
)
async def ultimatetictactoeCommand(ctx, user: discord.User):
    if not user == None:
        if not (ctx.author.bot or user.bot):
            global loop
            loop.create_task(
...
@client.command(
    name="connectfour",
    help="Drop pieces from the top, they fall down to the bottom, attempt to get four in a row before your opponent does",
    usage="@user *width *height\n* = optional",
    aliases=["c4"],
    description="A simple game of connect four",
)
async def connectfourCommand(ctx, user: discord.User, width: int = 7, height: int = 6):
    if not user == None:
...
@client.command(
    name="chess",
    help="https://en.wikipedia.org/wiki/Chess\nNOTE: THIS VERSION OF CHESS DOES NOT HAVE CASTLING",
    usage="@user",
    description="Its chess",
)
async def chessCommand(ctx, user: discord.User):
    if not user == None:
        if not (ctx.author.bot or user.bot):
            global loop
            loop.create_task(
...
@client.command(
    ...
    help="Scissors beats Paper beats Rock beats Scissors, try to guess which one will make you win against your opponent",
    usage="@user *rounds\n* = optional",
    aliases=["rps"],
    description="A simple game of Rock Paper Scissors",
)
async def rockpaperscissorsCommand(ctx, user: discord.User, rounds: int = 1):
    if not user == None:
        if not (ctx.author.bot or user.bot):
            global loop
            loop.create_task(
...
@client.command(hidden=True, aliases=["r", "restart", "shutdown"])
async def restartCommand(ctx, *, args: str = ""):
    if ctx.author.id == owner:
        ...
        if (activity(True) and not "y" in args) or (training and not "t" in args):
            ...
            if activity(True) and not "y" in args:
                ...
                components = [
                    ActionRow(
                        Button(label="Yes", style=ButtonStyle.green, id="restart:yes"),
                        Button(label="No", style=ButtonStyle.red, id="restart:no"),
                    )
                ]
            if training and not "t" in args:
                string = (
                    string.replace("are you sure you want to restart?", "")
                    + "im training right now, either abort training or wait until later"
                )
                components = []
                delete_after = 5
            ...
            if len(components) > 0 and not training:
                while True:
                    interaction = await client.wait_for(
                        "button_click",
                        check=lambda i: i.component.id.startswith("restart"),
                    )
...
async def dotrainingc4(ctx, amount, smartopponent):
    ...
    if amount > 0:
        try:
            await client.change_presence(
                activity=discord.Activity(
                    ...
                    name=f"Connect Four {amount} times",
                )
            )
        except Exception as e:
            print(str(e))
        await ctx.message.reply(
            f"{amount} games should take approximately {secondstotime(amount / gps)}"
        )
        ...
        for i in range(0, amount):
            winner, moves = await connectFourTraining(
                ctx,
                f"{ctx.guild.id}{ctx.channel.id}{ctx.message.id}:training:{i}",
            ...
            if (i + 1) % 50 == 0:
                ...
                print(i + 1, end=":\t")
                print(
                    f"{(locallean / completed) * 100:.2f}%\t{(lean / completed) * 100:.2f}%",
                    end="\t|\t",
                )
                print(
                    f"{localmoves / 50:.2f}\t{totalmoves / completed:.2f}", end="\t|\t"
                )
                print(f"{ties}", end="\t|\t")
        ...
        try:
            await client.change_presence(
                activity=discord.Activity(
                    type=discord.ActivityType.playing, name=f"Connect Four indefinitely"
                )
            )
        except Exception as e:
            print(str(e))
    ...
    await ctx.message.reply(
        f"Finished {completed} games in {secondstotime(elapsed)} ({completed / elapsed:.2f} games per second)\nwon {(lean / completed) * 100:.2f}% of the time {ties} were ties\navg moves = {totalmoves / completed:.2f}"
    )
    ...
    if singleplayer or specialdata["ai"]:
        ...
        if gametype == "forfeit":
            await ctx.channel.send(
                f"{user.mention}, are you sure you want to forfeit?",
                components=[
                    ActionRow(
                        Button(
                            id=f"{gameid}:accept", label="Yes", style=ButtonStyle.green
                        ),
                        Button(
                            id=f"{gameid}:decline", label="No", style=ButtonStyle.red
                        ),
                    )
                ],
            )
            ...
            await ctx.send(
                f"{user.mention}, you have been challenged to {roundstring} of {makereadable(gametype)} by {ctx.author.mention}",
                components=[
                    ActionRow(
                        Button(
                            id=f"{gameid}:accept",
        ...
        while True:
            interaction = await client.wait_for(
                "button_click", check=lambda i: i.component.id.startswith(gameid)
            )
            if interaction.user.id == user.id:
                ...
                if response == "accept":
                    if gametype == "forfeit":
                        ...
                        loop.create_task(
                            tictactoeGame(ctx, gameid, players, singleplayer)
                        )
                    elif gametype == "uttt":
                        loop.create_task(
                            ultimatetictactoeGame(ctx, gameid, players, singleplayer)
                        )
                    elif gametype == "c4":
                        loop.create_task(
                            connectFourGame(
                        ...
                        loop.create_task(
                            rockpaperscissorsGame(ctx, gameid, players, rounds)
                        )
                    elif gametype == "chess":
                        loop.create_task(chessGame(ctx, gameid, players, singleplayer))
                await interaction.message.delete()
...
async def tictactoeGame(ctx, gameid, players, singleplayer):
    ...
    currentTTTgames[gameid] = tictactoe.tictactoe(
        ctx, players, gameid, singleplayer=singleplayer
    )
    ...
    while True:
        ...
        if player[0]:
            response = await currentTTTgames[gameid].set(
                interaction.component.id.split(":")[-1]
            )
            if response[0]:
                try:
                    await interaction.respond(type=6)
...
async def ultimatetictactoeGame(ctx, gameid, players, singleplayer):
    ...
    currentUltimateTTTgames[gameid] = ultimateTTT.ultimateTTT(
        ctx, players, gameid, True, singleplayer=singleplayer
    )
    ...
    while True:
        ...
        if player[0]:
            try:
                ...
                print(str(e))
            response = await currentUltimateTTTgames[gameid].set(
                interaction.component.id.split(":")[-1]
            )
            if response[0]:
...
async def connectFourGame(ctx, gameid, players, singleplayer, ai, dimensions):
    ...
    currentC4games[gameid] = connectfour.connectfour(
        ctx, players, gameid, singleplayer, ai, dimensions
    )
    win = False
    ...
    while True:
        interaction = await client.wait_for(
            "select_option", check=lambda i: i.component[0].value.startswith(gameid)
        )
        ...
        if player[0]:
            try:
                ...
                print(str(e))
            response = await currentC4games[gameid].set(
                int(interaction.component[0].value.split(":")[-1])
            )
            if currentC4games[gameid].failed:
                currentC4games.pop(gameid)
                return
...
async def rockpaperscissorsGame(ctx, gameid, players, rounds):
    ...
    currentRPSgames[gameid] = rockpaperscissors.rockpaperscissors(
        ctx, players, gameid, rounds
    )
    ...
    while True:
        ...
        if player[0]:
            response = await currentRPSgames[gameid].set(
                interaction.component.id.split(":")[-1], interaction.user
            )
            if response[0]:
                try:
                    await interaction.respond(type=6)
...
async def chessGame(ctx, gameid, players, singleplayer):
    global currentChessGames
    currentChessGames[gameid] = chess.chess(ctx, players, gameid, singleplayer)
    ...
    while True:
        ...
        if interaction.component[0].value.endswith("forfeit"):
            player = currentChessGames[gameid].isvalidplayer(
                interaction.user.id, checkTurn=False
            )
            if player[0]:
                global loop
                loop.create_task(
                    confirmationmessage(
                        interaction, interaction.user, gameid, "forfeit", False
                    )
                )
                try:
                    await interaction.respond(type=6)
            ...
            if player[0]:
                response = await currentChessGames[gameid].set(
                    interaction.component[0].value.split(":")[-1]
                )
                if response[0]:
                    try:
                        await interaction.respond(type=6)
...
async def gameoverchecker(client, type):
    while True:
        if type == "button_click":
            interaction = await client.wait_for(
                type,
                check=lambda i: (not i.component.id.split(":")[0] in currentTTTgames)
                and (not i.component.id.split(":")[0] in currentUltimateTTTgames)
                and (not i.component.id.split(":")[0] in currentRPSgames)
                and (not i.component.id.split(":")[0] in currentConfirming)
                and (not i.component.id.startswith("restart")),
            )
        elif type == "select_option":
            interaction = await client.wait_for(
                type,
                check=lambda i: (
                    not i.component[0].value.split(":")[0] in currentC4games
                )
                and (not i.component[0].value.split(":")[0] in currentChessGames),
            )

        await interaction.respond(
            content="this was lost to the sands of time (probably a restart, my bad, sorry)"
        )
        await interaction.message.delete()


async def connectFourTraining(
    ctx, gameid, players, singleplayer, ai, dimensions, sendmessage, smartopponent
):
    global currentC4games
    currentC4games[gameid] = connectfour.connectfour(
        ctx,
    ...
    while True:
        ...
        if win:
            winner = currentC4games[gameid].inverseplayerhelper[
                currentC4games[gameid].winner
            ]
            ...
            if not training:
                try:
                    await client.change_presence(
                        activity=discord.Activity(
                            type=discord.ActivityType.playing,
                            name=f"for {secondstotime(elapsed)}",
                        )
                    )
                except Exception as e:
                    print(str(e))
