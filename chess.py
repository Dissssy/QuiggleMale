...
class chess:
    ...
    pieceEmoteStrings = [
        [
            [
                "<:wPr:878232501603172423>",
                "<:wRr:878232501905154108>",
                "<:wNr:878232501728968785>",
                "<:wBr:878232501926121503>",
                "<:wQr:878232501976449085>",
                "<:wKr:878232501921927188>",
            ],
            [
                "<:bPr:878232501582200833>",
                "<:bRr:878232501540257813>",
                "<:bNr:878232501909352478>",
                "<:bBr:878232501640917032>",
                "<:bQr:878232501531869246>",
                "<:bKr:878232504543359016>",
            ],
        ],
        [
            [
                "<:wPb:878236200450801714>",
                "<:wRb:878236200417230858>",
                "<:wNb:878236200274628658>",
                "<:wBb:878236735060975626>",
                "<:wQb:878236200362709043>",
                "<:wKb:878236200480170044>",
            ],
            [
                "<:bPb:878236200165580910>",
                "<:bRb:878236200136216576>",
                "<:bNb:878236200173989958>",
                "<:bBb:878236200304005121>",
                "<:bQb:878236200249458708>",
                "<:bKb:878236200228503552>",
            ],
        ],
    ]
    promotions = [3, 2, 1, 4]
    ...
    def __init__(self, ctx, players, gameid, singleplayer):
        ...
        self.gamestate = [
            ...
            [self.piece(0, 0) for i in range(8)],
            [None for i in range(8)],
            [None for i in range(8)],
            [None for i in range(8)],
            [None for i in range(8)],
            [self.piece(0, 1) for i in range(8)],
            [
                self.piece(1, 1),
                self.piece(2, 1),
                self.piece(3, 1),
            ...
            for j in range(8):
                if gamestate[i][j] is not None:
                    if not gamestate[i][j]["type"] == 5:
                        (
                            gamestate[i][j]["moves"],
                            gamestate[i][j]["movecount"],
                            gamestate[i][j]["captures"],
                        ) = self.checkMoves([i, j], gamestate, checking)
                    else:
                        dolast[(gamestate[i][j]["color"] + self.currentplayer) % 2] = [
                            i,
                            j,
                        ]
        for piece in dolast:
            if piece is not None:
                (
                    gamestate[piece[0]][piece[1]]["moves"],
                    gamestate[piece[0]][piece[1]]["movecount"],
                    gamestate[piece[0]][piece[1]]["captures"],
                ) = self.checkMoves([piece[0], piece[1]], gamestate, checking)
        return gamestate
                ...
                if gamestate[i][j] is not None:
                    if not gamestate[i][j]["type"] == 5:
                        (
                            ...
                            gamestate[i][j]["movecount"],
                            gamestate[i][j]["captures"],
                        ) = self.checkMovesCopy([i, j], gamestate, checking)
                    else:
                        dolast[(gamestate[i][j]["color"] + self.currentplayer) % 2] = [
                            i,
                            j,
        ...
        for piece in dolast:
            if piece is not None:
                (
                    ...
                    gamestate[piece[0]][piece[1]]["movecount"],
                    gamestate[piece[0]][piece[1]]["captures"],
                ) = self.checkMovesCopy([piece[0], piece[1]], gamestate, checking)
        return gamestate
    ...
    def checkMoves(self, piece, gamestate, checking=False):
        ...
        if gamestate[piece[0]][piece[1]]["type"] == 0:
            ...
            if piece[0] + direction >= 0 and piece[0] + direction <= 7:
                if gamestate[piece[0] + direction][piece[1]] is None:
                    ...
                    if (
                        gamestate[piece[0]][piece[1]]["firstmove"]
                        and gamestate[piece[0] + direction + direction][piece[1]]
                        is None
                    ):
                ...
                for i in range(2):
                    if piece[1] + ((i * 2) - 1) >= 0 and piece[1] + ((i * 2) - 1) <= 7:
                        captures[
                            str([piece[0] + direction, piece[1] + ((i * 2) - 1)])
                        ] = True
                        if (
                            gamestate[piece[0] + direction][piece[1] + ((i * 2) - 1)]
                            is not None
                        ):
                            if (
                                not gamestate[piece[0] + direction][
                                    piece[1] + ((i * 2) - 1)
                                ]["color"]
                                == gamestate[piece[0]][piece[1]]["color"]
                            ):
                                moves.append(
                                    [piece[0] + direction, piece[1] + ((i * 2) - 1)]
                                )
        elif gamestate[piece[0]][piece[1]]["type"] == 1:
            for direction in self.rookDirections:
                for i in range(8):
                    if (
                        piece[0] + (direction[0] * (i + 1)) >= 0
                        and piece[0] + (direction[0] * (i + 1)) <= 7
                        and piece[1] + (direction[1] * (i + 1)) >= 0
                        and piece[1] + (direction[1] * (i + 1)) <= 7
                    ):
                        captures[
                            str(
                                [
                                    piece[0] + (direction[0] * (i + 1)),
                                    piece[1] + (direction[1] * (i + 1)),
                                ]
                            )
                        ] = True
                        if (
                            gamestate[piece[0] + (direction[0] * (i + 1))][
                                piece[1] + (direction[1] * (i + 1))
                            ]
                            is not None
                        ):
                            if (
                                not gamestate[piece[0] + (direction[0] * (i + 1))][
                                    piece[1] + (direction[1] * (i + 1))
                                ]["color"]
                                == gamestate[piece[0]][piece[1]]["color"]
                            ):
                                ...
                                if (
                                    ...
                                    captures[
                                        str(
                                            [
                                                piece[0] + (direction[0] * (i + 2)),
                                                piece[1] + (direction[1] * (i + 2)),
                                            ]
                                        )
                                    ] = True
                            break
            ...
            for direction in self.knightDirections:
                if (
                    piece[0] + direction[0] >= 0
                    and piece[0] + direction[0] <= 7
                    and piece[1] + direction[1] >= 0
                    and piece[1] + direction[1] <= 7
                ):
                    captures[
                        str([piece[0] + direction[0], piece[1] + direction[1]])
                    ] = True
                    if (
                        gamestate[piece[0] + direction[0]][piece[1] + direction[1]]
                        is not None
                    ):
                        if (
                            not gamestate[piece[0] + direction[0]][
                                piece[1] + direction[1]
                            ]["color"]
                            == gamestate[piece[0]][piece[1]]["color"]
                        ):
                            moves.append(
                                [piece[0] + direction[0], piece[1] + direction[1]]
                            )
                    else:
        ...
        if gamestate[piece[0]][piece[1]]["type"] == 4:
            ...
            moveCount = len(moves)
        if (
            gamestate[piece[0]][piece[1]]["type"] == 0
            and piece[0] == (gamestate[piece[0]][piece[1]]["color"] * -7) + 7
        ):
            self.promote = piece
    ...
    def updatePiece(self, piece):
        self.gamestate[piece[0]][piece[1]]["name"] = self.intToType[
            self.gamestate[piece[0]][piece[1]]["type"]
        ]
        ...
        for i in range(8):
            ...
            for j in range(8):
                ...
                if self.gamestate[i][j] is None:
                    ...
                    string += self.pieceEmoteStrings[color][
                        self.gamestate[i][j]["color"]
                    ][self.gamestate[i][j]["type"]]
            if self.intToEmote[0]:
                string += f"{self.intToEmote[i + 1]}"
            string += "\n"
        if self.intToEmote[0]:
        ...
        for i in range(8):
            for j in range(8):
                if self.gamestate[i][j] is not None:
                    if (
                        f"{self.gamestate[i][j]['color']}{self.gamestate[i][j]['type']}"
                        not in pieces
                    ):
                        pieces[
                            f"{self.gamestate[i][j]['color']}{self.gamestate[i][j]['type']}"
                        ] = Image.open(
                            f'./resources/{self.gamestate[i][j]["color"]}{self.gamestate[i][j]["type"]}.png'
                        ).convert(
                            "RGBA"
                        )
                    image.paste(
                    ...
                    if self.gamestate[i][j]["color"] == self.currentplayer:
                        if self.gamestate[i][j]["type"] == 4:
                            for moves in self.gamestate[i][j]["moves"]:
                                if space in moves:
                                    piecesCapture[self.gamestate[i][j]["type"]].append(
                                        [[i, j], space]
                                    )
                        else:
    ...
    def constructoptions(
        self, piece=None, promote=False, moveQueen=None, check=None, checkType=None
    ):
        options = []
        end = False
        if not check:
            if piece is None:
                for i in range(8):
                    for j in range(8):
                        if self.gamestate[i][j] is not None:
                            if (
                                self.gamestate[i][j]["movecount"] > 0
                                and self.gamestate[i][j]["color"] == self.currentplayer
                            ):
                                options.append(
                                    SelectOption(
                                        label=f"{self.intToLetter[j]}{i + 1} : {self.gamestate[i][j]['name']}",
                                        value=f"{self.gameid}:select|{i}{j}",
                                    )
                                )
            else:
                if not promote:
                    if not self.gamestate[piece[0]][piece[1]]["type"] == 4:
                        for move in self.gamestate[piece[0]][piece[1]]["moves"]:
                            ...
                            options.append(
                                SelectOption(
                                    label=f"{self.intToLetter[piece[1]]}{piece[0] + 1} to {self.intToLetter[move[1]]}{move[0] + 1}{poop}",
                                    value=f"{self.gameid}:move|{piece[0]}{piece[1]}|{move[0]}{move[1]}",
                                )
                            )
                    else:
                        if moveQueen is None:
                            for i in range(8):
                                ...
                                if len(moves) > 0:
                                    ...
                                    options.append(
                                        SelectOption(
                                            label=f"{self.intToLetter[piece[1]]}{piece[0] + 1} towards {self.intToLetter[move[1]]}{move[0] + 1}",
                                            value=f"{self.gameid}:queen|{piece[0]}{piece[1]}|{i}",
                                        )
                                    )
                        else:
                            for move in self.gamestate[piece[0]][piece[1]]["moves"][
                    ...
                    options.append(
                        SelectOption(
                            label=f"Back",
                            value=f"{self.gameid}:move|{piece[0]}{piece[1]}|back",
                        )
                    )
                else:
                    for promotion in self.promotions:
                        options.append(
                            SelectOption(
                                label=f"promote {self.intToLetter[piece[1]]}{piece[0] + 1} to {self.intToType[promotion]}",
                                value=f"{self.gameid}:promote|{piece[0]}{piece[1]}|{promotion}",
                            )
                        )
            if len(options) == 0:
                options.append(
                    SelectOption(label=f"Game over", value=f"{self.gameid}:gameover")
                )
                end = True
        else:
            if not self.cantUncheck:
                if checkType is None:
                    ...
                    for i in range(6):
                        if not i == 5:
                            if len(self.checkpieces[i]) > 0:
                                options.append(
                                    SelectOption(
                                        label=f"Escape check with a {self.intToType[i]}",
                                        value=f"{self.gameid}:checktype|{i}",
                                    )
                                )
                    for move in self.gamestate[piece[0]][piece[1]]["moves"]:
                        poop = ""
                        ...
                        options.append(
                            SelectOption(
                                label=f"Move King {self.intToLetter[piece[1]]}{piece[0] + 1} to {self.intToLetter[move[1]]}{move[0] + 1}{poop}",
                                value=f"{self.gameid}:move|{piece[0]}{piece[1]}|{move[0]}{move[1]}",
                            )
                        )
                else:
                    for piecemove in self.checkpieces[checkType]:
                        ...
                        options.append(
                            SelectOption(
                                label=f"{self.intToLetter[apiece[1]]}{apiece[0] + 1} to {self.intToLetter[move[1]]}{move[0] + 1}{poop}",
                                value=f"{self.gameid}:move|{apiece[0]}{apiece[1]}|{move[0]}{move[1]}",
                            )
                        )
                    options.append(
                        SelectOption(
    ...
    async def constructmessage(
        ...
        if not end:
            options, end = self.constructoptions(
                piece,
                promote=promote,
                moveQueen=moveQueen,
                check=check,
        ...
        if not end:
            options.append(
                SelectOption(label=f"Forfeit", value=f"{self.gameid}:forfeit")
            )
            components = [
                ActionRow(
                    Select(
                        options=options,
                        id=f"{self.gameid}",
                        placeholder=f"Make your move {self.players[self.currentplayer].name}",
                    )
                )
            ]
        else:
        ...
        if self.message == None:
            with BytesIO() as image_binary:
                ...
                self.message = await self.ctx.send(
                    f"{boardstring}{self.intToColor[self.currentplayer]}s turn ({self.players[self.currentplayer].mention})\n-- {fart}{piss}",
            ...
            if self.winner is None:
                if (
                    moveQueen is None
                    and checkType is None
                    and not back
                    and (piece is None or check is not None)
                ):
                ...
                with BytesIO() as image_binary:
                    ...
                    image_binary.seek(0)
                    self.message = await self.ctx.send(
                        f"{boardstring}WINNER: {self.intToColor[(self.currentplayer + 1) % 2]} ({self.winner.mention})",
                        components=components,
                        file=discord.File(fp=image_binary, filename="image.png"),
                    )
    ...
    async def set(self, value):
        ...
        if values[0] == "select":
            ...
            if values[2] == "back":
                if self.check is None:
                    ...
                    await self.constructmessage(
                        self.kings[self.currentplayer], back=True, check=True
                    )
            else:
                self.check = None
                self.checkpieces = None
                self.gamestate[int(values[2][0])][int(values[2][1])] = self.gamestate[
                    int(values[1][0])
                ][int(values[1][1])]
                self.gamestate[int(values[1][0])][int(values[1][1])] = None
                self.gamestate[int(values[2][0])][int(values[2][1])][
                    "firstmove"
                ] = False
                if self.gamestate[int(values[2][0])][int(values[2][1])]["type"] == 5:
                    self.kings[
                        self.gamestate[int(values[2][0])][int(values[2][1])]["color"]
                ...
                if self.promote == None:
                    if self.check == None:
                        ...
                        self.gamestate = self.checkAllMoves(self.gamestate)
                        await self.constructmessage(
                            self.kings[self.currentplayer], check=True
                        )
                else:
                    await self.constructmessage(self.promote, promote=True)
        elif values[0] == "promote":
            self.gamestate[int(values[1][0])][int(values[1][1])]["type"] = int(
                values[2][0]
            )
            self.gamestate[int(values[1][0])][int(values[1][1])][
                "name"
            ] = self.intToType[int(values[2][0])]
            self.promote = None
            self.gamestate = self.checkAllMoves(self.gamestate)
            await self.swapPlayers()
            await self.constructmessage()
            ...
            await self.constructmessage(
                [int(values[1][0]), int(values[1][1])], moveQueen=int(values[2])
            )
        elif values[0] == "checktype":
            await self.constructmessage(
                self.check, check=True, checkType=int(values[1])
            )
        if self.winner == None:
            return [False, [True, ""]]
        else:
