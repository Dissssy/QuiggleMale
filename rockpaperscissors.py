...
class rockpaperscissors:
    ...
    async def _construct_message(self):
        ...
        components = [
            ActionRow(
                Button(
                    label="👊",
                    id=f"{self.gameid}:rock",
                    style=ButtonStyle.red,
                    disabled=gotwinner,
                ),
                Button(
                    label="✋",
                    id=f"{self.gameid}:paper",
                    style=ButtonStyle.green,
                    disabled=gotwinner,
                ),
                Button(
                    label="✌",
                    id=f"{self.gameid}:scissors",
        ...
        if self.message == None:
            self.message = await self.ctx.send(
                f"{ping[0]} : {self.wins[0]}\n{ping[1]} : {self.wins[1]}\nround {self.currentround}/{self.rounds}",
            ...
            if self.totalwinner is None:
                ...
                if self.totalwinner == "tie":
                    ...
                    await self.message.delete()
                    self.message = await self.ctx.send(
                        f"winner: {self.totalwinner.mention}\n{self.wins[self._convert_player_to_index(self.totalwinner)]} to {self.wins[(self._convert_player_to_index(self.totalwinner) + 1) % 2]}",
                        components=components,
                    )
    ...
    async def set(self, selection, player):
        if self.selections[self._convert_player_to_index(player)] is None:
            ...
            return [
                False,
                f"You've already made a selection this round, you picked {self.selections[self._convert_player_to_index(player)]}",
            ]
    ...
    def _checkgamewon(self):
        if self.wins[0] > floor(self.rounds / 2) or self.wins[1] > floor(
            self.rounds / 2
        ):
