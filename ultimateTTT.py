    ...
    def makebutton(self, x, y):
        if self.maingame:
            ...
            return Button(
                ...
                disabled=self.gamestate[x][y].tie
                or not (self.winner == None)
                or not (state == ButtonStyle.grey),
            )
        else:
                ...
                for j in range(3):
                    for k in range(3):
                        ...
                        if winner == None or self.winner is not None:
                            globalboard[j + (h * 3)][k + (i * 3)] = self.gamestate[h][
                                i
                            ].gamestate[j][k]
                        elif winner == "tie":
        ...
        if self.maingame:
            ...
            if self.winner == None:
                if self.selection == None:
                    ...
                    await self.gamestate[self.selection[0]][
                        self.selection[1]
                    ]._construct_message()
                    if self.winner == None:
                        return [False, [True, ""]]
                    ...
                    await self.gamestate[self.selection[0]][self.selection[1]].set(
                        index
                    )
                    await self._swapPlayers()
                    self.selection = [int(index[0]), int(index[1])]
                    if (
                        self.gamestate[self.selection[0]][self.selection[1]].winner
                        == None
                    ):
                        self._checkgamewon()
